...
#include <sstream>
  ...
  using namespace RooStats;
   ...
   void constrained_ignoreobs_scan( const char* wsfile = "outputfiles/ws.root",
                          const char* new_poi_name="mu_bg_4b_msig_met1",
                          const char* ignore_obs_name="N_4b_msig_met1",
     ...
     //-- Construct the new POI parameter.
      ...
      if ( new_poi_rar == 0x0 ) {
       ...
       RooAbsReal* rar_ignore_pdf(0x0) ;
       TString ignorepdfname( ignore_obs_name ) ;
       ignorepdfname.ReplaceAll("N_","pdf_") ;
       rar_ignore_pdf = ws -> pdf( ignorepdfname ) ;
       if ( rar_ignore_pdf == 0x0 ) {
          printf("\n\n *** Can't find pdf for ignore observable : %s \n\n", ignorepdfname.Data() ) ; return ;
       }
       RooFormulaVar* rfv_minuitvar_unbiased_unconstrained = new RooFormulaVar("minuitvar_unbiased_unconstrained",
                     "@0+log(@1)", RooArgList( *nll, *rar_ignore_pdf ) ) ;
       ...
       RooFitResult* rfr_uc = rminuit_ub_uc->fit("mr") ;
       ...
       RooAbsReal* plot_var = new RooFormulaVar("plot_var", plotvar_formula,
           RooArgList( *nll,
                       *rar_ignore_pdf
                       ) ) ;
    ...
    //----------------------------------------------------------------------------------------------
      ...
      //--- Main canvas
       ...
       hscan->SetXTitle("4b,SIG background events") ;
       hscan->SetYTitle("-2 ln(#Delta L)") ;
  ...
  //   sprintf( outrootfile, "%s/scan-ignoreobs-%s.root", outputdir.Data(), new_poi_name ) ;

  //   char outpdffile[10000] ;
  //   sprintf( outpdffile, "%s/scan-ignoreobs-%s.pdf", outputdir.Data(), new_poi_name ) ;

       char endstring[1000] ;

       TString outrootfile( wsfile ) ;
       outrootfile.ReplaceAll( "/ws-", "/scan-ignoreobs-" ) ;
       sprintf( endstring, "--%s.root", new_poi_name ) ;
       outrootfile.ReplaceAll( ".root", endstring ) ;

       TString outpdffile( wsfile ) ;
       outpdffile.ReplaceAll( "/ws-", "/scan-ignoreobs-" ) ;
       sprintf( endstring, "--%s.pdf", new_poi_name ) ;
       outpdffile.ReplaceAll( ".root", endstring ) ;
